...
- **Prefill 阶段**：计算用户输入的 prompt 并填充到 KV Cache 中。 Prefill 阶段的计算密度是比较高的，一般一次只需要处理一个请求。如果存在多个请求，则按 FIFO 顺序排队处理即可；
- **Decode 阶段**：基于已有的 KV Cache 序列，生成新的 token 并更新 KV Cache。每次只生成一个 token 是无法充分利用 GPU 算力的，因此往往需要将多个请求合并成 Batch 进行批量处理，来增加计算密度。

由于 Prefill 和 Decode 两个阶段的特点不同，Scheduler 需要做一些协调工作：

- 跟踪每个请求的处理状态，对尚未 Prefill 过的请求跑一把 Prefill；
- 从已完成 Prefill 的请求中，组成一个尽可能大的 batch，喂给 GPU 跑 Decode；

此外，Scheudler 也是内存分配与释放的发起者，Prefill 前，需要为 Prefill 需要的 Block 发起内存分配，Decode 期间，也随着生成出来的序列的增长，而分配 Block。

随着 Decode 的进行，如果 KV Cache 内存不足，Scheduler 也需要驱逐一部分请求的 Block，来腾出空间来。
