...
### Local Cache of Topology Information
...
2. Update the configuration center with the new chain topology `[N1, N2, N3, N4]` and mark the chain as "Locked" status;
...
The second step of marking the chain as "Locked" status also doesn't require real-time reads/writes from consensus services, but can rely on asynchronous subscription. CRAQ's strong consistency mechanism requiring all nodes to write before considering data Committed means we only need "any node" to stop writes to achieve locking.
