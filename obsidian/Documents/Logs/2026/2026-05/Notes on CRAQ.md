...
从某种意义上说，CRAQ 已经成为对象存储复制领域里相当流行的工程方案。而 CRAQ 可以看作是对传统的链式复制的一个简单的增强。不过与 Raft 这类共识算法不同，CRAQ 只关注数据复制本身，它仍然需要显式依赖一个外部的强一致配置中心（如 etcd 或 ZooKeeper）来管理复制链的拓扑信息。
