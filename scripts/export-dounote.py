...
class AnnotationExporter(object):
    ...
    def export(self, user_name, dir_path="./booknotes"):
        ...
        for book_title, annotations in book_annotations_pairs:
            ...
            gen_path = u"%s/%s.md" % (
                dir_path,
                self._cook_target_path(book_title, last_date),
            )
            with open(gen_path, "w+") as f:
                print(u"render %s" % gen_path)
                md_content = self._render_annotations_of_one_book(
    ...
    def _fetch_with_pagination(self, user_name, start, count=100):
        ...
        resp = requests.get(
            "https://api.douban.com/v2/book/user/%s/annotations?count=%d&start=%d"
            % (user_name, count, start)
        )
