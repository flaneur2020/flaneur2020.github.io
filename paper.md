...
需要留意的是，除四个段描述符之外，fleurix的GDT中也带有一个TSS描述符，其选择符为(`_TSS`)。Intel引入TSS机制的动机为实现硬件的任务切换，每个任务拥有一个TSS，在进程切换时，将当前进程的**所有**上下文保存在TSS中。比起软件的任务切换，硬件任务切换的开销相对比较大，而且没有调试与优化的余地。fleurix采用了软件的任务切换机制，保留一个TSS仅仅用于保存中断处理时ss0与esp0两个寄存器的值，在CPU通过中断门或者自陷门转移控制权时，据此获取内核栈的位置。
