...
文件是对I/O的抽象，而文件系统提供了文件的组织方式。fleurix实现了minix v1文件系统，主要是因为它结构简单、易于实现，而且在开发环境中也有丰富的工具可供使用，如`mkfs.minix`、`fsck.minix`等。
...
在fleurix中有两个数据结构与超级块相关：`struct d_super`与`struct super`，分别对应磁盘上与内存中超级块的表示。后者多了几个字段来表示挂载信息。
