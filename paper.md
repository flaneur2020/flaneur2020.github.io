...
fleurix采用传统UNIX的优先级调度算法。在进程结构中，调度相关的字段只有三个(取值范围皆为-126到127)：

+ `p_cpu`：已执行的时间片计数；
+ `p_nice`：用户通过`nice()`系统调用设置的微调；
+ `p_pri`：进程的优先值，优先值越小优先级越高。

内核会定时增加`p_cpu`的值，同时每隔一定时间便依据`p_cpu`与`p_nice`重新计算`p_pri`(可见于`src/kern/sched.c`中的`setpri()`)。公式大致为：

    p->p_pri = p->p_cpu/16 + PUSER + p->p_nice;
    p->p_pri = min(p->p_pri, 127);
    p->p_pri = max(p->p_pri, -126);

随着时间的增加，当前进程的优先级会越来越低，直到低于其它的任何进程。在这时调用`swtch()`，便可以找出当前优先级最高的进程并切换。
