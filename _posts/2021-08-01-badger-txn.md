...
![](/images/badger-txn-conflict.png)
...
2. 循环为 pendingWrites 和 duplicateWrites 中的 Entry 的 version 绑定 commitTs，并使存储的 key 绑定 commitTs
3. 调用 txn.db.sendToWriteCh(entries) 使写入缓冲进入落盘写入
4. 等待落盘完成后，通知 orc.doneCommit(commitTs)，移动 txnMark 的点位
