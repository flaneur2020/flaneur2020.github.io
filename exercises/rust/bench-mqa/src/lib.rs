...
impl Work {
    ...
    fn multi_head_attention_layout1(&self) -> Vec<f32> {
        let mut attn_scores = vec![0.0; self.seq_len * self.n_heads];
        ...
        for h in 0..self.n_heads {
            for pos in 0..self.seq_len {
                ...
                attn_scores[pos * self.n_heads + h] = strided_vec_dot(
                    &self.kcache,
                    &self.q,
                    offset_a,
                    offset_b,
        ...
        let attn_scores_strides = [self.n_heads, 1]; // seq_len, n_heads
        let attn_scores_strides = [attn_scores_strides[1], attn_scores_strides[0]]; // n_heads, seq_len
        for h in 0..self.n_heads {
